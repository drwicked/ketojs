...
module.exports = function(App, protocol, location){
    ...
    App.emit('route.start', { app: App, request, response })
    ...
    // set default header
		...
		const location = request.url ? url.parse(request.url) : '' ;
		const port     = host.split(':')[1] || protocol.globalAgent.defaultPort;
		const hostname = utils.isset(port) ? host : host + ':' + protocol.globalAgent.defaultPort ;

		// get app (host controller) handling this hostname
		const app = App.hosts[hostname] || App.hosts[hostname+':'+port] || App.hosts['0.0.0.0:'+port] || App.hosts[port]

		// if the app (host controller) exists and it has routes for this method
		if (app && app.routes && app.routes[method]) {

      // set default headers
			for (const key in app.defaultHeaders) {
        response.setHeader(key, app.defaultHeaders[key])
			...
			if(!path){                                     // dynamic path
				for(const index in routes){                      // loop trough all method routes
					const item = new Array(routes[index][0])
