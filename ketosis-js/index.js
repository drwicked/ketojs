...
const Server = ( options = {} ) => {
  ...
  //  Get project path
  // -----------------------------------------------------------------------
  const path = options.path || require('path').dirname(require('callsite')()[1].getFileName());

  // -----------------------------------------------------------------------
  //  Event: app.create
  // -----------------------------------------------------------------------
  module.exports.emit('init', { path, options })
  ...
  // -----------------------------------------------------------------------
  const app = new App(path, options)

  // -----------------------------------------------------------------------
  //  Server is an Event Emitter
  // -----------------------------------------------------------------------
  const appevents = new EventEmitter({ wildcards: true });
  app.on = appevents.on;
  app.one = appevents.once;
  app.many = appevents.many;
  app.off = appevents.off;
  app.emit = appevents.emit;
  app.onAny = appevents.onAny;

  // -----------------------------------------------------------------------
  //  Attach HTTP Protocol Handler by default
  // -----------------------------------------------------------------------  
  app.protocol('http', httpProtocol)
  module.exports.emit('create', { path, options, app })
...
const events = new EventEmitter({ wildcards: true });
